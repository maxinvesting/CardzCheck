...
-- Thread status
DO $$ BEGIN
  CREATE TYPE thread_status AS ENUM ('open', 'needs_response', 'awaiting_buyer', 'resolved', 'archived');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
-- Message category / topic
DO $$ BEGIN
  CREATE TYPE message_category AS ENUM ('question', 'offer', 'shipping', 'complaint', 'return_refund', 'other');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
-- Direction
DO $$ BEGIN
...
create index if not exists idx_message_threads_status on message_threads(user_id, status);
create index if not exists idx_message_threads_last_message on message_threads(user_id, last_message_at desc);
-- RLS
...
DROP POLICY IF EXISTS "Users can manage messages in own threads" ON messages;
...
DROP POLICY IF EXISTS "Users can manage AI drafts for own threads" ON ai_reply_drafts;
