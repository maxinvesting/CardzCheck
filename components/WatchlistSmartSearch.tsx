...
interface WatchlistSmartSearchProps {
  isOpen: boolean;
  onClose: () => void;
...
type WatchlistSearchResult = SmartSearchResult;
...
export default function WatchlistSmartSearch({
  isOpen,
  onClose,
  onCardSelected,
}: WatchlistSmartSearchProps) {
  const [query, setQuery] = useState("");
  const [searchLoading, setSearchLoading] = useState(false);
  const [searchResult, setSearchResult] = useState<WatchlistSearchResult | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/watchlist/search?q=${encodeURIComponent(query.trim())}`);
  ...
  const buildCardDataFromCandidate = (candidate: SmartSearchCandidate) => {
    if (!searchResult) return null;

    const parsed: ParsedSearch = searchResult.parsed.original;

    return {
      player_name: parsed.player_name || query,
      year: candidate.year || parsed.year,
      set_brand:
        (candidate.brand && candidate.line ? `${candidate.brand} ${candidate.line}` : candidate.brand || candidate.line) ||
        parsed.set_name,
      card_number: candidate.cardNumber || parsed.card_number,
      parallel_variant: candidate.parallel || parsed.parallel_type,
      condition: parsed.grade,
    };
  };

  const handleAddCandidateToWatchlist = (candidate: SmartSearchCandidate) => {
    const cardData = buildCardDataFromCandidate(candidate);
    if (!cardData) return;
    onCardSelected(cardData);
    handleClose();
  };
  ...
  const renderCandidate = (candidate: SmartSearchCandidate, index: number, bucket: "exact" | "close") => {
    const confidencePct = candidate.confidence !== undefined ? Math.round(candidate.confidence * 100) : null;
    ...
    return (
      <button
        ...
        type="button"
        onClick={() => handleAddCandidateToWatchlist(candidate)}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-white dark:bg-gray-900 rounded-2xl shadow-xl flex-shrink-0">
        {/* Header */}
          ...
          <div ref={wrapperRef} className="relative">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search card database or get one eBay comp... (e.g., Jordan 1986 PSA 10)"
                ...
                {suggestions.map((suggestion, index) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-900 dark:text-white">{suggestion.label}</span>
                    </div>
                    {suggestion.metadata && (
                      <span className="text-xs text-gray-500 dark:text-gray-400">{suggestion.metadata}</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-4 pt-4 border-t border-gray-200 dark:border-gray-800">
              ...
              <div className="space-y-2">
                ...
                {searchResult.exact.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {searchResult.exact.map((candidate, index) => renderCandidate(candidate, index, "exact"))}
                  </div>
                )}
              </div>
              ...
              {searchResult.close.length > 0 && (
                <div className="space-y-2">
                  ...
                  {showCloseMatches && (
                    <div className="space-y-2">
                      {searchResult.close.map((candidate, index) => renderCandidate(candidate, index, "close"))}
                    </div>
                  )}
                </div>
              )}
