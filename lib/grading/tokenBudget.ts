...
export type GradeTokenBudgetReservation = GradeTokenBudgetStatus & {
  ...
  /**
   * Set when `reserve_grade_token_budget` RPC failed (e.g. migration not applied on the project).
   * Callers can fall back to {@link checkGradeTokenBudget} + direct usage recording.
   */
  rpcError?: boolean;
};
...
export async function reserveGradeTokenBudget(
  ...
  if (error) {
    console.error("[token-budget] Failed to reserve grade token budget:", error.message, error);
    return {
      allowed: false,
      rpcError: true,
