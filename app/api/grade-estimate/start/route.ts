  ...
  try {
    ...
    if (!isTestMode()) {
      ...
      if (proAccess.tier === "free") {
        ...
        if (budgetReservation.rpcError) {
          // Reservation RPC missing or failing (e.g. migration not applied). Fall back to a
          // read-only budget check and record usage via increment_grade_token_usage after the run.
          const check = await checkGradeTokenBudget(user.id, proAccess.tier);
          if (!check.allowed) {
            return NextResponse.json(
              {
                error: check.reason ?? "Monthly scanning budget reached.",
                code: "BUDGET_EXCEEDED",
                budgetCents: check.budgetCents,
                spentCents: check.spentCents,
                reservedCents: check.reservedCents,
              },
              { status: 429 }
            );
          }
          budgetReservationId = null;
        } else if (!budgetReservation.allowed || !budgetReservation.reservationId) {
          return NextResponse.json(
            {
